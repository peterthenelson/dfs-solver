...
use crate::core::{Attribution, ConstraintResult, Error, Feature, Index, Key, RankingInfo, State, Stateful, Unscored, VBitSet, VSetMut};
...
pub struct XSumChecker<const MIN: u8, const MAX: u8, const N: usize, const M: usize> {
    ...
    xsum_head_feature: Key<Feature>,
    xsum_ln_possibilities_feature: Key<Feature>,
    xsum_tail_feature: Key<Feature>,
    xsum_sum_over_attr: Key<Attribution>,
    xsum_sum_bad_attr: Key<Attribution>,
    xsum_sum_if_attr: Key<Attribution>,
    xsum_len_if_attr: Key<Attribution>,
}
...
impl <const MIN: u8, const MAX: u8, const N: usize, const M: usize> XSumChecker<MIN, MAX, N, M> {
    pub fn new(xsums: Vec<XSum<MIN, MAX, N, M>>) -> Self {
        ...
        XSumChecker {
            ...
            xsum_head_feature: Key::register(XSUM_HEAD_FEATURE),
            xsum_ln_possibilities_feature: Key::register(XSUM_LN_POSSIBILITIES_FEATURE),
            xsum_tail_feature: Key::register(XSUM_TAIL_FEATURE),
            xsum_sum_over_attr: Key::register(XSUM_SUM_OVER_ATTRIBUTION),
            xsum_sum_bad_attr: Key::register(XSUM_SUM_BAD_ATTRIBUTION),
            xsum_sum_if_attr: Key::register(XSUM_SUM_INFEASIBLE_ATTRIBUTION),
            xsum_len_if_attr: Key::register(XSUM_LEN_INFEASIBLE_ATTRIBUTION),
        }
    }
}
