use rand::Rng;
use variant_sudoku::{bench::Bench, core::VSet, kropki::{kropki_black_possible, kropki_black_possible_chain}, sudoku::StdVal};
...
KropkiBench for Kropki<MIN, MAX> {
    fn run_kropki_black_possible(&self, _: &mut ChaCha20Rng) {
        touch_set::<MIN, MAX, _>(&kropki_black_possible::<MIN, MAX>().get());
    }
    ...
    fn run_kropki_black_possible_chain(&self, rng: &mut ChaCha20Rng) {
        ...
        touch_set::<MIN, MAX, _>(&kropki_black_possible_chain::<MIN, MAX>().get(n_mutually_visible, len_from_end));
    }
}
